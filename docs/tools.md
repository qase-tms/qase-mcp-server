...
| `qase_attachment_upload` | Upload one or more files and get back their hashes, which every `attachments` field accepts. This is the only way to obtain such a hash — the endpoint needs `multipart/form-data`, which `qase_api` cannot send. Pass `files` to send several in a single request; hashes come back in the same order. Use `file_base64` whenever the server is not on the same machine as the file, including the hosted connector, where it is the only option. **Qase accepts at most 20 files, 32 MB per file and 128 MB per request** — a request over any of those is refused before anything is uploaded, so split it across calls. A 507 from the API means the team account is out of attachment storage. | `code`, `files` (array, 1-20 — each: `filename`, `file_base64`, `file_path`); or the single-file form: `filename`, `file_base64` (base64 content), `file_path` (absolute path, local servers only), `file` (deprecated alias accepting either) | **core** |
